...
internal class Bullet : MonoBehaviour
{
    ...
    private void OnTriggernter(Collision collision)
    {
        

        //if (collision.gameObject.CompareTag(_opponent.obj.tag))
        //{
    ...
    private bool CheckCollision(Collider collision, Character hitCharacter, Type intendedType)
    {
        if (hitCharacter != null && hitCharacter.GetType() == intendedType)
        {
            ...
            Debug.Log("Shot " + hitCharacter.Obj.name + " for " + _damage + " damage");
            return true;
        }
        return false;
    }
